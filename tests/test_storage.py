...
# --- D-033: the server's write-only depth token -------------------------------------------


def _depth_write_env(monkeypatch):
    monkeypatch.setenv("FRONTDOOR_DEPTH_BUCKET", DEPTH)
...
def test_depth_write_creds_are_a_different_token_from_the_harness_read_token(monkeypatch):
    """A separate token, not the same one reused.

    If these ever resolve to one credential, the server can read depth and D-033's guarantee is
...
def test_depth_write_creds_report_which_variable_is_missing(monkeypatch):
    ...
    with pytest.raises(StorageError, match="FRONTDOOR_DEPTH_WRITE_ACCESS_KEY"):
        storage.load_depth_write_creds()


@mock_aws
def test_the_write_probe_fails_loudly_when_the_token_can_read(monkeypatch):
    """moto grants full access, so a read succeeds — which is exactly the leak being detected.

    This is the check working: under a correctly scoped R2 token the read is denied and the probe
    returns. Here it is not denied, and the probe must refuse to call that a pass.
    """
    _both_env(monkeypatch)
    _depth_write_env(monkeypatch)
    _create_buckets()
...
@mock_aws
def test_the_write_probe_passes_when_reads_are_denied(monkeypatch):
    """The intended shape: PUT works, GET is refused."""
    _both_env(monkeypatch)
    _depth_write_env(monkeypatch)
    _create_buckets()

    denied = storage.StorageDenied("denied")

    class WriteOnly(storage.ObjectStore):
