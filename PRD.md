...
| D-031 **(AMENDED — see D-044)** | **AMENDMENT TO D-026 (2026-09-02) — the measure server runs on a PAID host.** Fly.io `shared-cpu-1x`/256 MB in `sjc`, about $2/month. Object storage stays free. | Measured, not estimated: the image serves `GET /health` capped at 256 MB using 69 MiB, and Fly's smallest always-on machine is 256 MB at ~$2 against Render's 512 MB at $7. **D-026's "billing must be incapable of starting silently" clause is not waived.** D-044 restates the mechanism: prepaid credits that suspend at zero balance, plus an advisory billing alert, recorded in `docs/server-deploy.md`. Authorises the server host and nothing else. Live at `https://frontdoor-measure.fly.dev`. **Reported as an amendment at Demo Day.** |
| D-032 **(CORRECTED — see D-040, D-045)** | **Depth delivery is enabled on James's iPhone 17 Pro (`iPhone18,1`)** (2026-09-02). | The decision selected James's LiDAR-equipped phone and authorised depth capture and storage only. D-045 records the 2026-09-04 probe: relative stereo disparity from `builtInDualWideCamera`, persisted as `DepthFloat32`, not LiDAR range. A frame without depth-carried calibration is rejected for missing intrinsics. James runs Windows, so the mandatory 7-day re-sign (D-025) needs a Mac owner present. |
...
| D-036 **(CORRECTED — see D-040, D-045)** | **One capture device, depth enabled, no caliper** (2026-09-02). Capture and the live demo run on **James's iPhone 17 Pro (`iPhone18,1`) with LiDAR hardware**; generic depth delivery is enabled and supported delivered data is persisted as `DepthFloat32`; **no caliper is used**. Supersedes D-003. | One operator with one phone is the shortest path to a uniform dataset. After applying D-040's alias rule, `device_model` is a single physical-device value. The phone is an accepted, unmitigated single point of failure. D-045 restates clause 1: depth is captured as stereo disparity, not LiDAR range. **No caliper means no instrument ground truth**, so the §2 criterion is untested rather than relaxed and the planned LiDAR comparison has no metric reference. The screening study is unaffected. |
...
| D-044 | **AMENDMENT TO D-031 (2026-09-04) — D-026's no-silent-billing clause is met by prepaid credits, not a Fly spend limit.** | Fly.io documents no organisation spend limit, cap, or budget control (PR #200 review F1). Alert emails are advisory. Prepaid credits suspend the account at zero balance. D-031's paid-host decision is unchanged. TICK-261 (#260). |
| D-045 | **AMENDMENT TO D-032 and D-036 (2026-09-04) — captured depth is relative stereo disparity, not LiDAR range.** | The 2026-09-04 probe on James's iPhone 17 Pro (`iPhone18,1`) measured `builtInDualWideCamera` disparity (`hdis`, `accuracy=relative`), persisted as `DepthFloat32`. `builtInLiDARDepthCamera` is unused because it delivers no calibration. D-015 and D-020 are untouched. PR #255. TICK-261 (#260). |
